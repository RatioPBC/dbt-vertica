...
{% macro vertica__get_catalog_volatile(information_schema, schemas) -%}
    {% set query %}
        with tables as (
            select * from (
                select
            ...
            select
                ...
                comment
            from v_internal.vs_comments_p com
            where objectoid in (select id from tables)
        ), view_comments as (
            select
                oid as id,
                ...
                comment
            from v_internal.vs_comments_p com
            where objectoid in (select id from views)
        ), tables_with_comments as (
            select
                t.*,
                tc.comment as table_comment
            from tables t
            left join table_comments tc on t.id = tc.table_id
        ), views_with_comments as (
            select
                v.*,
                vc.comment as table_comment
            from views v
            left join view_comments vc on v.id = vc.view_id
        ), tables_and_views as (
            select * from tables_with_comments twc
            union all
            select * from views_with_comments vwc
        )
        select
            tav.*,
            com.comment as column_comment
        from tables_and_views tav
        left join v_internal.vs_sub_comments com
            on com.objectoid = split_part(tav.column_id, '-', 1)::int
            and com.childobject = tav.column_name
        ;
    {%- endset -%}
    {{ return(run_query(query)) }}
{%- endmacro %}
